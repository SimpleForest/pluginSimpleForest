...
SF_StepPrincipalDirection::SF_StepPrincipalDirection(CT_StepInitializeData& dataInit) : SF_AbstractStepFeature(dataInit) {}

SF_StepPrincipalDirection::~SF_StepPrincipalDirection() {}

QString
SF_StepPrincipalDirection::getStepDescription() const
{
  return tr("PrincipalDirection");
}

QString
SF_StepPrincipalDirection::getStepDetailledDescription() const
{
...
QString
SF_StepPrincipalDirection::getStepURL() const
{
  return tr("");
}
...
QStringList
SF_StepPrincipalDirection::getStepRISCitations() const
{
...
void
SF_StepPrincipalDirection::createInResultModelListProtected()
{
  CT_InResultModelGroupToCopy* resModel = createNewInResultModelForCopy(DEF_IN_RESULT, tr("Point Cloud"), "", false);
...
void
SF_StepPrincipalDirection::createOutResultModelListProtected()
{
  CT_OutResultModelGroupToCopyPossibilities* resOutModel = createNewOutResultModelToCopy(DEF_IN_RESULT);
  if (resOutModel != NULL) {
    resOutModel->addItemModel(DEF_IN_GRP_CLUSTER, m_outCloudItem, new CT_PointsAttributesColor(), tr("Principal Component"));
  }
}

void
SF_StepPrincipalDirection::createPostConfigurationDialogCitationSecond(CT_StepConfigurableDialog* configDialog)
{
  addCitationRaumonen(configDialog);
}

void
SF_StepPrincipalDirection::compute()
{
...
void
SF_StepPrincipalDirection::createPostConfigurationDialogBeginner(CT_StepConfigurableDialog* configDialog)
{}

void
SF_StepPrincipalDirection::createPostConfigurationDialogExpert(CT_StepConfigurableDialog* configDialog)
{
  ...
  configDialog->addDouble("Then the cloud is downscaled with "
                          ...
                          0.5,
                          3,
                          m_voxelSizeDownscaling);
  configDialog->addText("Then this step computes the <b>principal direction</b>, e.g. the second "
                        "normal derivate, for a point cloud.");
  configDialog->addDouble(
    "The [<em><b>normal radius</b></em>]:", " (m)", 0.01, 0.5, 2, m_normalRadius, 1, "Used for the normal computation.");
  ...
  configDialog->addText("The output is stored color coded on the backscaled cloud.");
}

std::vector<std::pair<pcl::PointCloud<SF_PointNormal>::Ptr, std::vector<size_t>>>
SF_StepPrincipalDirection::voxelizeInput(SF_ConverterCTCloudToPCLCloud<SF_PointNormal>& converter)
{
...
void
SF_StepPrincipalDirection::processInput(CT_ResultGroup* outResult)
{
  ...
  while (!isStopped() && outResIt.hasNext()) {
    ...
    CT_AbstractItemDrawableWithPointCloud* ctCloud = (CT_AbstractItemDrawableWithPointCloud*)group->firstItemByINModelName(
    ...
    std::vector<std::pair<pcl::PointCloud<SF_PointNormal>::Ptr, std::vector<size_t>>> clusters = voxelizeInput(converter);
    QList<SF_ParameterSetPrincipalDirection<SF_PointNormal>> paramList;
    std::for_each(clusters.begin(),
                  clusters.end(),
                  [&paramList, &param](const std::pair<pcl::PointCloud<SF_PointNormal>::Ptr, std::vector<size_t>>& cluster) {
                    SF_ParameterSetPrincipalDirection<SF_PointNormal> paramCluster = param;
                    paramCluster.m_paramVoxelGridDownscaling.m_cloud = cluster;
                    paramList.append(paramCluster);
                  });
    pcl::PointCloud<SF_PointNormal>::Ptr cloud(new pcl::PointCloud<SF_PointNormal>);
    pcl::PointCloud<pcl::PrincipalCurvatures>::Ptr featureCloud(new pcl::PointCloud<pcl::PrincipalCurvatures>);
    QFuture<void> future = QtConcurrent::map(paramList, SF_AdapterPrincipalDirection());
    setProgressByFuture(future, 10, 85);
    std::for_each(
      paramList.begin(), paramList.end(), [&cloud, &featureCloud](SF_ParameterSetPrincipalDirection<SF_PointNormal>& param) {
    ...
    CT_ColorCloudStdVector* colors = new CT_ColorCloudStdVector(cloudIn->points.size());
    size_t index = 0;
    std::for_each(cloudIn->points.begin(), cloudIn->points.end(), [&kdtree, &featureCloud, &index, colors](SF_PointNormal& point) {
      ...
      if (kdtree.nearestKSearch(point, 1, indices, distances)) {
        ...
        Eigen::Vector3f vec(feature.principal_curvature_x, feature.principal_curvature_y, feature.principal_curvature_z);
        if (feature.principal_curvature_z < 0) {
          vec = Eigen::Vector3f(-feature.principal_curvature_x, -feature.principal_curvature_y, -feature.principal_curvature_z);
        }
    ...
    CT_PointsAttributesColor* colorAttribute = new CT_PointsAttributesColor(
      m_outCloudItem.completeName(), outResult, ctCloud->getPointCloudIndexRegistered(), colors);
