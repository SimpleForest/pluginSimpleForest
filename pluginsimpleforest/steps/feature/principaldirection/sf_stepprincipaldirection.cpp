...
void SF_StepPrincipalDirection::createPostConfigurationDialogBeginner(CT_StepConfigurableDialog *configDialog)
{

}

void SF_StepPrincipalDirection::createPostConfigurationDialogExpert(CT_StepConfigurableDialog *configDialog)
{
    ...
    configDialog->addDouble("Then the cloud is downscaled with [<em><b>voxelGridCluster cell size</b></em>]:",
                            " (m).",
                            0.01,
                            0.5,
                            3,
                            m_voxelSizeDownscaling);
    configDialog->addText("Then this step computes the <b>principal direction</b>, e.g. the second normal derivate, for a point cloud.");
...
SF_StepPrincipalDirection::processInput(CT_ResultGroup * outResult)
{
    ...
    while(!isStopped() && outResIt.hasNext()) {
       ...
       (CT_AbstractItemDrawableWithPointCloud*) group->firstItemByINModelName(this,
                                                                              DEF_IN_CLOUD_SEED);
        ...
        std::for_each(clusters.begin(), clusters.end(), [&paramList, &param]
                      (const std::pair<pcl::PointCloud<SF_PointNormal>::Ptr, std::vector<size_t> > &cluster){
        ...
        setProgressByFuture(future,
                            10,
                            85);
        std::for_each(paramList.begin(), paramList.end(), [&cloud, &featureCloud]
                      (SF_ParameterSetPrincipalDirection<SF_PointNormal> &param){
        ...
        size_t index = 0;
        std::for_each(cloudIn->points.begin(), cloudIn->points.end(), [&kdtree, &featureCloud, &index, colors]
...
//                col.r() = (std::sqrt((vecNorm[0])*(vecNorm[0]) +(vecNorm[1])*(vecNorm[1]))*126);
//                col.g() = (std::sqrt((vecNorm[0])*(vecNorm[0]) +(vecNorm[1])*(vecNorm[1]))*126);
//                col.b() = (std::abs((vecNorm[2]*250)));
            }
        });
