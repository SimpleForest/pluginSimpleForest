...
QString SF_StepDTM::getStepDescription() const { return tr("DTM generation"); }

QString SF_StepDTM::getStepDetailledDescription() const {
  return tr("DTM generation - uses a pyramidal MLESAC plane fitting apporach "
...
void SF_StepDTM::createInResultModelListProtected() {
  ...
  res_model->addGroupModel("", DEF_IN_SCENE,
                           CT_AbstractItemGroup::staticGetType(),
                           tr("Input Scene Group"), "",
                           CT_InAbstractGroupModel::CG_ChooseOneIfMultiple);
  res_model->addItemModel(DEF_IN_SCENE, DEF_IN_SCENE_CLOUD,
                          CT_Scene::staticGetType(), tr("Input Scene"));
}

void SF_StepDTM::createPostConfigurationDialogExpert(
  ...
  configDialog->addDouble("For each of the downscaled points its normal is "
                          ...
                          " (m). ", 0.025, 0.5, 3, _radiusNormal);
  configDialog->addDouble("The cell size of the DTM is supposed to be  ",
                          " (m).", 0.025, 0.5, 3, _cellSize);
  configDialog->addInt("For IDW interpolation the following number of nearest "
                       "neighbors is needed ",
                       ".", 1, 99, _idwNeighbors);
  configDialog->addInt("For median interpolation the following number of "
                       "nearest neighbors is needed ",
                       ".", 1, 99, _medianNeighbors);
  configDialog->addDouble("The angle between a plane normal and the parent "
                          "plane normal has to be smaller than ",
                          " (°).", 0.5, 45, 1, _angle);
}

void SF_StepDTM::createPostConfigurationDialogBeginner(
    CT_StepConfigurableDialog *configDialog) {
  configDialog->addStringChoice("Choose the slope of the terrain", "",
...
void SF_StepDTM::createOutResultModelListProtected() {
  ...
  if (res_modelw != NULL) {
    res_modelw->addGroupModel(DEF_IN_SCENE, _outGroundGRP,
                              new CT_StandardItemGroup(), tr("Terrain"));
    res_modelw->addItemModel(_outGroundGRP, _outDTM, new CT_Image2D<float>(),
                             tr("DTM"));
    res_modelw->addItemModel(_outGroundGRP, _outCloud, new CT_Scene(),
                             tr("Ground points"));
  }
}
...
void SF_StepDTM::copyCroppedHeights(
    pcl::PointCloud<pcl::PointXYZINormal>::Ptr groundCloud,
    std::shared_ptr<CT_Image2D<float>> dtmPtr, CT_Image2D<float> *CTDTM) {
...
void SF_StepDTM::compute() {
  ...
  CT_StandardItemGroup *terrainGrp =
      new CT_StandardItemGroup(_outGroundGRP.completeName(), out_result);
  root->addGroup(terrainGrp);
  pcl::PointCloud<pcl::PointXYZINormal>::Ptr groundCloud =
      createGroundCloud(out_result, terrainGrp);
  SF_DTM<pcl::PointXYZINormal> sfDTM(groundCloud, _angle, _cellSize, out_result,
                                     _outDTMDummy);
  std::shared_ptr<CT_Image2D<float>> dtmPtr = sfDTM.DTM();
  CT_Image2D<float> *dtm = CT_Image2D<float>::createImage2DFromXYCoords(
      _outDTMDummy.completeName(), out_result, dtmPtr->minX() + _translate(0),
      dtmPtr->minY() + _translate(1), dtmPtr->maxX() + _translate(0),
      dtmPtr->maxY() + _translate(1), dtmPtr->resolution(), _translate(2), 1337,
      0);
  ...
  CT_Image2D<float> *dtmTrueResolution =
      CT_Image2D<float>::createImage2DFromXYCoords(
          _outDTMDummy.completeName(), out_result,
          dtmPtr->minX() + _translate(0), dtmPtr->minY() + _translate(1),
          dtmPtr->maxX() + _translate(0), dtmPtr->maxY() + _translate(1),
          _radiusNormal, _translate(2), 1337, 0);
  SF_ConverterCTToPCLDTM dtmConverterTrueResolution(_translate,
                                                    dtmTrueResolution);
  std::shared_ptr<SF_ModelDTM> dtmModelTrueResolution =
      dtmConverterTrueResolution.dtmPCL();
  dtmModel->interpolateIDW(_idwNeighbors, dtmModelTrueResolution);
  CT_Image2D<float> *dtmMedianSmoothed =
      CT_Image2D<float>::createImage2DFromXYCoords(
          _outDTM.completeName(), out_result, dtmPtr->minX() + _translate(0),
          dtmPtr->minY() + _translate(1), dtmPtr->maxX() + _translate(0),
          dtmPtr->maxY() + _translate(1), _radiusNormal, _translate(2), 1337,
          0);
  SF_ConverterCTToPCLDTM dtmConverterMedianSmoothed(_translate,
                                                    dtmMedianSmoothed);
  std::shared_ptr<SF_ModelDTM> dtmModelMedianSmoothed =
      dtmConverterMedianSmoothed.dtmPCL();
  dtmModelTrueResolution->interpolateMedian(_medianNeighbors,
                                            dtmModelMedianSmoothed);
  pcl::PointCloud<pcl::PointXYZ>::Ptr cloud =
      dtmModelMedianSmoothed->getCloud3D();
...
void SF_StepDTM::computeNormals(
    pcl::PointCloud<pcl::PointXYZINormal>::Ptr downscaledCloud) {
...
SF_StepDTM::convert(CT_Scene *scene) {
  ...
  pcl::PointCloud<pcl::PointXYZINormal>::Ptr cloud =
      converter.cloudTranslated();
  return cloud;
}

CT_Scene *
SF_StepDTM::addGroundCloudToResult(CT_PointCloudIndexVector *mergedClouds,
                                   CT_StandardItemGroup *root,
                                   CT_ResultGroup *outResult) {
  CT_Scene *scene(
      new CT_Scene(_outCloud.completeName(), outResult,
                   PS_REPOSITORY->registerPointCloudIndex(mergedClouds)));
...
pcl::PointCloud<pcl::PointXYZINormal>::Ptr
SF_StepDTM::createGroundCloud(CT_ResultGroup *outResult,
                              CT_StandardItemGroup *terrainGrp) {
  CT_Scene *scene = mergeIndices(outResult, terrainGrp, DEF_IN_GRP_CLUSTER,
                                 DEF_IN_CLOUD_SEED);
