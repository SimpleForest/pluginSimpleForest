...
SF_StepDTM::createPostConfigurationDialogExpert(CT_StepConfigurableDialog* configDialog)
{
  ...
  configDialog->addDouble("For each of the downscaled points its normal is "
                          ...
                          3,
                          _radiusNormal);
  configDialog->addDouble("The cell size of the DTM is supposed to be  ", " (m).", 0.025, 0.5, 3, _cellSize);
...
SF_StepDTM::compute()
{
  ...
  SF_DTM<pcl::PointXYZINormal> sfDTM(groundCloud, _angle, _cellSize, out_result, _outDTMDummy);
...
CT_Scene*
SF_StepDTM::addGroundCloudToResult(CT_PointCloudIndexVector* mergedClouds, CT_StandardItemGroup* root, CT_ResultGroup* outResult)
{
