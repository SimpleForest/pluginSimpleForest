...
QString
SF_RadiusOutlierFilterStep::getStepDescription() const
{
  return tr("Radius Outlier Filter");
}

QString
SF_RadiusOutlierFilterStep::getStepDetailledDescription() const
{
...
QString
SF_RadiusOutlierFilterStep::getStepURL() const
{
...
QStringList
SF_RadiusOutlierFilterStep::getStepRISCitations() const
{
...
void
SF_RadiusOutlierFilterStep::createInResultModelListProtected()
{
...
void
SF_RadiusOutlierFilterStep::createPostConfigurationDialogExpert(CT_StepConfigurableDialog* configDialog)
{
  configDialog->addDouble("Looks for each point at its numbers of neighbors in range ", "", 0.01, 4.1, 3, _radius);
  configDialog->addInt("A point is eliminated if it contains less than.", " Points", 2, 99999, _minPts);
}

void
SF_RadiusOutlierFilterStep::createPostConfigurationDialogBeginner(CT_StepConfigurableDialog* configDialog)
{
...
void
SF_RadiusOutlierFilterStep::createOutResultModelListProtected()
{
  CT_OutResultModelGroupToCopyPossibilities* resModelw = createNewOutResultModelToCopy(DEF_IN_RESULT);
  if (resModelw != NULL) {
    resModelw->addItemModel(DEF_IN_GRP_CLUSTER, m_outCloudItem, new CT_PointsAttributesColor(), tr("Point Density"));
    resModelw->addGroupModel(DEF_IN_GRP_CLUSTER, _outGrp, new CT_StandardItemGroup(), tr("Radius Outlier Filter"));
...
void
SF_RadiusOutlierFilterStep::writeOutputPerScence(CT_ResultGroup* outResult, size_t i)
{
...
void
SF_RadiusOutlierFilterStep::writeOutput(CT_ResultGroup* outResult)
{
...
void
SF_RadiusOutlierFilterStep::compute()
{
  ...
  QFuture<void> future = QtConcurrent::map(_paramList, SF_RadiusOutlierFilterAdapter());
...
void
SF_RadiusOutlierFilterStep::writeLogger()
{
  if (!_paramList.empty()) {
...
void
SF_RadiusOutlierFilterStep::adaptParametersToExpertLevel()
{
  if (!_isExpert) {
    _radius = 0.03;
...
void
SF_RadiusOutlierFilterStep::createParamList(CT_ResultGroup* outResult)
{
