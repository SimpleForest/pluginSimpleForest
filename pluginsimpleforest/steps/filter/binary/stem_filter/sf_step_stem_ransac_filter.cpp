...
void SF_Step_Stem_RANSAC_Filter::adaptParametersToExpertLevel() {
    if(!_isExpert) {
        if(_choice == _less) {
            _x = 0;
            _y = 0;
...
void SF_Step_Stem_RANSAC_Filter::write_output_per_scence(CT_ResultGroup* out_result, size_t i) {
    SF_Param_Stem_RANSAC_Filter param = _paramList.at(i);
...
void SF_Step_Stem_RANSAC_Filter::compute() {
    ...
    QFuture<void> future = QtConcurrent::map(_paramList,SF_Step_Stem_Filter_RANSAC_Adapter() );
    setProgressByFuture(future,10,85);
    write_output(out_result);
}

void SF_Step_Stem_RANSAC_Filter::writeLogger() {
    if(!_paramList.empty()) {
