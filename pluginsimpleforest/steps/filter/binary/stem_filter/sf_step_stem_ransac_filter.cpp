...
*****************************************************************************/
#include "sf_step_stem_ransac_filter.h"

#include "sf_step_stem_filter_ransac_adapter.h"
#include <QtConcurrent/QtConcurrent>

SF_Step_Stem_RANSAC_Filter::SF_Step_Stem_RANSAC_Filter(CT_StepInitializeData &data_init): SF_Abstract_Filter_Binary_Step(data_init) {
}

SF_Step_Stem_RANSAC_Filter::~SF_Step_Stem_RANSAC_Filter() {
}

QString SF_Step_Stem_RANSAC_Filter::getStepDescription() const {
    return tr("Stem Filter by RANSAC fitting");
}

QString SF_Step_Stem_RANSAC_Filter::getStepDetailledDescription() const {
...
QString SF_Step_Stem_RANSAC_Filter::getStepURL() const {
    return tr("https://www.youtube.com/watch?v=5i6_Rtv-xEw");
}

CT_VirtualAbstractStep* SF_Step_Stem_RANSAC_Filter::createNewInstance(CT_StepInitializeData &dataInit) {
    return new SF_Step_Stem_RANSAC_Filter(dataInit);
}

QStringList SF_Step_Stem_RANSAC_Filter::getStepRISCitations() const {
...
void SF_Step_Stem_RANSAC_Filter::createInResultModelListProtected() {
    ...
    res_model->addGroupModel("", DEF_IN_GRP_CLUSTER, CT_AbstractItemGroup::staticGetType(), tr("Cloud Grp "), "", CT_InAbstractGroupModel::CG_ChooseOneIfMultiple);
    res_model->addItemModel(DEF_IN_GRP_CLUSTER, DEF_IN_CLOUD_SEED, CT_Scene::staticGetType(), tr("Point Cloud"));
}

void SF_Step_Stem_RANSAC_Filter::createPostConfigurationDialogExpert(CT_StepConfigurableDialog *config_dialog) {
    config_dialog->addDouble("First the cloud is downscaled to a voxel size of  ",   " (m). " , 0.005,0.1,3,_voxel_size );
    ...
    config_dialog->addDouble("Into 1 meter horizontal slices a RANSAC cylinder is fitted with   ", "  (m). " , 0.01,0.5,2,_inlierDistance );
    config_dialog->addText(" inlier Distance. All inliers per slice are set to stem if the cylinder passes the following test:");
    config_dialog->addDouble("The angle for each point between this cylinder axis and the adjusted z axis is computed and is not allowed to deviate more than ", " " , 0.5,180,1,_angle );
...
void SF_Step_Stem_RANSAC_Filter::createPostConfigurationDialogBeginner(CT_StepConfigurableDialog *config_dialog) {
    config_dialog->addStringChoice("Choose how many points should be removed","",_non_expert_level, _choice);
    config_dialog->addText("For bended trees select a weaker filter level. Also select weaker level for worse clouds.");
}

void SF_Step_Stem_RANSAC_Filter::createOutResultModelListProtected() {
    CT_OutResultModelGroupToCopyPossibilities *res_modelw = createNewOutResultModelToCopy(DEF_IN_RESULT);
    if(res_modelw != NULL) {
        res_modelw->addGroupModel(DEF_IN_GRP_CLUSTER, _out_grp, new CT_StandardItemGroup(), tr ("Stem RANSAC filter") );
...
void SF_Step_Stem_RANSAC_Filter::adapt_parameters_to_expert_level() {
    if(!_is_expert) {
        if(_choice == _less) {
            _x = 0;
            _y = 0;
...
void SF_Step_Stem_RANSAC_Filter::write_output_per_scence(CT_ResultGroup* out_result, size_t i) {
    SF_Param_Stem_RANSAC_Filter param = _param_list.at(i);
...
void SF_Step_Stem_RANSAC_Filter::write_output(CT_ResultGroup* out_result) {
...
void SF_Step_Stem_RANSAC_Filter::compute() {
    ...
    QFuture<void> future = QtConcurrent::map(_param_list,SF_Step_Stem_Filter_RANSAC_Adapter() );
    set_progress_by_future(future,10,85);
    write_output(out_result);
}

void SF_Step_Stem_RANSAC_Filter::write_logger() {
...
void SF_Step_Stem_RANSAC_Filter::create_param_list(CT_ResultGroup * out_result) {
    ...
    while(!isStopped() && out_res_it.hasNext()) {
        ...
        SF_Param_Stem_RANSAC_Filter param;
        param._log = PS_LOG;
        param._x = _x;
        param._y = _y;
        param._z = _z;
