...
SF_StepStemRANSACFilter::SF_StepStemRANSACFilter(CT_StepInitializeData& dataInit) : SF_AbstractFilterBinaryStep(dataInit) {}

SF_StepStemRANSACFilter::~SF_StepStemRANSACFilter() {}

QString
SF_StepStemRANSACFilter::getStepDescription() const
{
  return tr("Stem Filter by RANSAC fitting");
}

QString
SF_StepStemRANSACFilter::getStepDetailledDescription() const
{
...
QString
SF_StepStemRANSACFilter::getStepURL() const
{
...
QStringList
SF_StepStemRANSACFilter::getStepRISCitations() const
{
...
void
SF_StepStemRANSACFilter::createInResultModelListProtected()
{
  ...
  resModel->addItemModel(DEF_IN_GRP_CLUSTER, DEF_IN_CLOUD_SEED, CT_Scene::staticGetType(), tr("Clouod to be denoised"));
}

void
SF_StepStemRANSACFilter::createPostConfigurationDialogExpert(CT_StepConfigurableDialog* configDialog)
{
  configDialog->addDouble("First the cloud is downscaled to a voxel size of  ", " (m). ", 0.005, 0.1, 3, _voxelSize);
  ...
  configDialog->addDouble(
    "Into 1 meter horizontal slices a RANSAC cylinder is fitted with   ", "  (m). ", 0.01, 0.5, 2, _inlierDistance);
  configDialog->addText(" inlier Distance. All inliers per slice are set to "
                        "stem if the cylinder passes the following test:");
  configDialog->addDouble("The angle for each point between this cylinder axis and the z axis is "
...
void
SF_StepStemRANSACFilter::createPostConfigurationDialogBeginner(CT_StepConfigurableDialog* configDialog)
{
...
void
SF_StepStemRANSACFilter::createOutResultModelListProtected()
{
  CT_OutResultModelGroupToCopyPossibilities* resModelw = createNewOutResultModelToCopy(DEF_IN_RESULT);
  if (resModelw != NULL) {
    resModelw->addGroupModel(DEF_IN_GRP_CLUSTER, _outGrp, new CT_StandardItemGroup(), tr("Stem RANSAC Filter"));
...
void
SF_StepStemRANSACFilter::adaptParametersToExpertLevel()
{
  if (!_isExpert) {
...
void
SF_StepStemRANSACFilter::writeOutputPerScence(CT_ResultGroup* outResult, size_t i)
{
...
void
SF_StepStemRANSACFilter::writeOutput(CT_ResultGroup* outResult)
{
...
void
SF_StepStemRANSACFilter::compute()
{
...
void
SF_StepStemRANSACFilter::writeLogger()
{
  if (!_paramList.empty()) {
...
void
SF_StepStemRANSACFilter::createParamList(CT_ResultGroup* outResult)
{
