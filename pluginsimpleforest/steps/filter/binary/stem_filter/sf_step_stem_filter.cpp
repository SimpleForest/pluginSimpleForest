...
void SF_Step_Stem_Filter::createOutResultModelListProtected() {
    ...
    if(res_modelw != NULL) {
        res_modelw->addGroupModel(DEF_IN_GRP_CLUSTER, _out_grp, new CT_StandardItemGroup(), tr ("stem filtered removal") );
...
void SF_Step_Stem_Filter::adaptParametersToExpertLevel() {
    if(!_isExpert) {
        if(_choice == _less) {
            _x = 0;
            _y = 0;
...
void SF_Step_Stem_Filter::compute() {
    ...
    QFuture<void> future = QtConcurrent::map(_paramList,SF_Step_Stem_Filter_Adapter() );
    setProgressByFuture(future,10,85);
    write_output(out_result);
}

void SF_Step_Stem_Filter::writeLogger() {
    if(!_paramList.empty()) {
