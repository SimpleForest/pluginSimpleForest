...
void SF_Step_Stem_Filter::createPostConfigurationDialogExpert(CT_StepConfigurableDialog *config_dialog) {
    config_dialog->addDouble("First the cloud is downscaled to a voxel size of  ", " " , 0.015,0.1,3,_voxel_size );
    config_dialog->addDouble(" (m). For each of the downscaled points its normal is computed with a range search of  ", " " , 0.025,0.2,3,_radius_normal );
    config_dialog->addDouble(" (m). Another neighborhood range search is performed on the downscaled points with range  ", " " , 0.05,0.5,3,_radius_growth_direction );
    config_dialog->addText(" (m). The second range should be larger than the first, which should be larger than the downscale size.");
    ...
    config_dialog->addText("Please read Raumonen <br>2013</br> for more information, this step is based on knowledge gained there.");
}
