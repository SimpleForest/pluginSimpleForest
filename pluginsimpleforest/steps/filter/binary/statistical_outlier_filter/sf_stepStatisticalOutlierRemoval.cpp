...
QString
SF_StepStatisticalOutlierRemoval::getStepDescription() const
{
  return tr("Statistical Outlier Filter");
}

QString
SF_StepStatisticalOutlierRemoval::getStepDetailledDescription() const
{
  return tr("Statistical Outlier Filter - Frontend to PCL filter. This step "
            "detects noise by analysing the density of each points "
            "neighborhood. Regions containing a small amount of point are "
            "likely to be tagged as noise. Can be repeated iteretivaly. A "
            "really high number of iterations (~50) combined with a high "
            "standard deviation multiplier (>= 4) performs most robust.");
}

QString
SF_StepStatisticalOutlierRemoval::getStepURL() const
{
  return tr("http://pointclouds.org/documentation/tutorials/statistical_outlier.php");
}

CT_VirtualAbstractStep*
SF_StepStatisticalOutlierRemoval::createNewInstance(CT_StepInitializeData& dataInit)
{
  return new SF_StepStatisticalOutlierRemoval(dataInit);
}

QStringList
SF_StepStatisticalOutlierRemoval::getStepRISCitations() const
{
...
void
SF_StepStatisticalOutlierRemoval::createInResultModelListProtected()
{
...
void
SF_StepStatisticalOutlierRemoval::createPostConfigurationDialogExpert(CT_StepConfigurableDialog* configDialog)
{
  configDialog->addInt("Looks for each point at its ", " closest neighbors", 1, 1000, _k);
  configDialog->addText("The average distance to the neighbor points is "
                        "computed for each point.");
  configDialog->addDouble("Assuming a normal distribution for all distances, all points further "
                          "away from the mean distance than ",
                          " ",
                          0.1,
                          10,
                          4,
                          _std_mult);
  configDialog->addText("times the standarddeviation are removed. As the standard distribution "
                        "parameters change, the procedure can be repeated multiple times.");
  configDialog->addInt("Please select the number of ", " iterations for the procedure", 1, 100, _iterations);
}

void
SF_StepStatisticalOutlierRemoval::createPostConfigurationDialogBeginner(CT_StepConfigurableDialog* configDialog)
{
...
void
SF_StepStatisticalOutlierRemoval::createOutResultModelListProtected()
{
  CT_OutResultModelGroupToCopyPossibilities* resModelw = createNewOutResultModelToCopy(DEF_IN_RESULT);
  if (resModelw != NULL) {
    resModelw->addGroupModel(DEF_IN_GRP_CLUSTER, _outGrp, new CT_StandardItemGroup(), tr("Statistical Outlier Filter"));
...
void
SF_StepStatisticalOutlierRemoval::adaptParametersToExpertLevel()
{
  if (!_isExpert) {
    _k = 9;
    _iterations = 15;
...
void
SF_StepStatisticalOutlierRemoval::writeOutputPerScence(CT_ResultGroup* outResult, size_t i)
{
...
void
SF_StepStatisticalOutlierRemoval::writeOutput(CT_ResultGroup* outResult)
{
...
void
SF_StepStatisticalOutlierRemoval::compute()
{
...
void
SF_StepStatisticalOutlierRemoval::writeLogger()
{
  if (!_paramList.empty()) {
...
void
SF_StepStatisticalOutlierRemoval::createParamList(CT_ResultGroup* outResult)
{
