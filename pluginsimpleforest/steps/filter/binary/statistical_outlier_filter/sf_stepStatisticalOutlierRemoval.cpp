...
CT_VirtualAbstractStep *SF_StepStatisticalOutlierRemoval::createNewInstance(
    CT_StepInitializeData &dataInit) {
...
void SF_StepStatisticalOutlierRemoval::createInResultModelListProtected() {
  ...
  resModel->addItemModel(DEF_IN_GRP_CLUSTER, DEF_IN_CLOUD_SEED,
                         CT_Scene::staticGetType(), tr("Cloud to be denoised"));
}

void SF_StepStatisticalOutlierRemoval::createPostConfigurationDialogExpert(
    CT_StepConfigurableDialog *configDialog) {
  configDialog->addInt("Looks for each point at its ", " closest neighbors", 1,
                       1000, _k);
  configDialog->addText("The average distance to the neighbor points is "
                        "computed for each point.");
  configDialog->addDouble(
      "Assuming a normal distribution for all distances, all points further "
      "away from the mean distance than ",
      " ", 0.1, 10, 4, _std_mult);
  configDialog->addText(
      "times the standarddeviation are removed. As the standard distribution "
      "parameters change, the procedure can be repeated multiple times.");
  configDialog->addInt("Please select the number of ",
                       " iterations for the procedure", 1, 100, _iterations);
}

void SF_StepStatisticalOutlierRemoval::createPostConfigurationDialogBeginner(
    CT_StepConfigurableDialog *configDialog) {
...
void SF_StepStatisticalOutlierRemoval::writeOutputPerScence(
    CT_ResultGroup *outResult, size_t i) {
...
void SF_StepStatisticalOutlierRemoval::compute() {
  ...
  QFuture<void> future =
      QtConcurrent::map(_paramList, SF_StatisticalOutlierRemovalAdapter());
...
void SF_StepStatisticalOutlierRemoval::createParamList(
