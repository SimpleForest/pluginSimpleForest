...
void SF_StepCutCloudAboveDTM::createInResultModelListProtected() {
  CT_InResultModelGroup *resModelDTM =
      createNewInResultModel(DEF_IN_RESULT_DTM, tr("DTM"));
  assert(resModelDTM != NULL);
  resModelDTM->setZeroOrMoreRootGroup();
  resModelDTM->addGroupModel(
      "", DEF_IN_DTMGRP, CT_AbstractItemGroup::staticGetType(),
      tr("DTM Grp In"), "", CT_InAbstractGroupModel::CG_ChooseOneIfMultiple);
  resModelDTM->addItemModel(DEF_IN_DTMGRP, DEF_IN_DTM,
                            CT_Image2D<float>::staticGetType(), tr("DTM"));
  ...
  res_model->addItemModel(DEF_IN_GRP_CLUSTER, DEF_IN_CLOUD_SEED,
                          CT_Scene::staticGetType(), tr("Cloud to be sliced"));
}

void SF_StepCutCloudAboveDTM::createPostConfigurationDialogExpert(
    CT_StepConfigurableDialog *configDialog) {
  configDialog->addDouble(
      "Over the input cloud is iterated. For each point the height <b>h</b> "
      "above the DTM is computed. If <b>h</b> is lower than ",
      " (m). ", -0.1, 50, 2, _cutHeight);
  configDialog->addText(
      " the point is put into the lower cloud, into the upper otherwise.");
}

void SF_StepCutCloudAboveDTM::createPostConfigurationDialogBeginner(
    CT_StepConfigurableDialog *configDialog) {}
...
void SF_StepCutCloudAboveDTM::createOutResultModelListProtected() {
  ...
  if (resModelw != NULL) {
    resModelw->addGroupModel(DEF_IN_GRP_CLUSTER, _outGrp,
                             new CT_StandardItemGroup(), tr("Vertical Slice"));
...
void SF_StepCutCloudAboveDTM::compute() {
  ...
  QFuture<void> future =
      QtConcurrent::map(_paramList, SF_StepCutCloudAboveDTMAdapter());
