...
QString SF_StepGroundFilter::getStepDetailledDescription() const {
  return tr(
      "Ground Point Filter - This Filter estimates for each point the normal. "
...
void SF_StepGroundFilter::createPostConfigurationDialogExpert(
  ...
  configDialog->addDouble(
      "The angle for each point between the normal and the z axis is computed "
      "and is not allowed to deviate more than ",
      " ", 0.5, 180, 1, _angle);
  configDialog->addText("degrees.");
}

void SF_StepGroundFilter::createPostConfigurationDialogBeginner(
    CT_StepConfigurableDialog *configDialog) {
...
void SF_StepGroundFilter::createOutResultModelListProtected() {
  ...
  if (resModelw != NULL) {
    ...
    resModelw->addGroupModel(DEF_IN_GRP_CLUSTER, _outGrp,
                             new CT_StandardItemGroup(), tr("Ground Filter"));
  ...
  if (!_isExpert) {
    ...
    if (_choiceNumberPoints == _few) {
      ...
      throw std::runtime_error(
          "SF_StepGroundFilter parameter adaptation error.");
    }
  }
}
...
void SF_StepGroundFilter::compute() {
  ...
  QFuture<void> future =
      QtConcurrent::map(_paramList, SF_StepGroundFilterAdapter());
