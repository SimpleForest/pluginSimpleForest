...
void SF_Step_Ground_Filter::adaptParametersToExpertLevel() {
    if(!_isExpert) {
        if(_choice == _less) {
            _x = 0;
            _y = 0;
...
void SF_Step_Ground_Filter::write_output_per_scence(CT_ResultGroup* out_result, size_t i) {
    SF_Param_Ground_Filter<SF_PointNormal> param = _paramList.at(i);
...
void SF_Step_Ground_Filter::compute() {
    ...
    QFuture<void> future = QtConcurrent::map(_paramList,SF_Step_Ground_Filter_Adapter() );
    setProgressByFuture(future,10,85);
    write_output(out_result);
}

void SF_Step_Ground_Filter::writeLogger() {
    if(!_paramList.empty()) {
...
void SF_Step_Ground_Filter::create_param_list(CT_ResultGroup * out_result) {
    ...
    while(!isStopped() && out_res_it.hasNext()) {
        ...
        SF_Param_Ground_Filter<SF_PointNormal> param;
