...
SF_EuclideanClusteringSegmentationStep::createNewInstance(
    CT_StepInitializeData &dataInit) {
...
void SF_EuclideanClusteringSegmentationStep::
  ...
  resModel->addGroupModel("", DEF_IN_SCENE,
                          CT_AbstractItemGroup::staticGetType(),
...
void SF_EuclideanClusteringSegmentationStep::createPostConfigurationDialog() {
  ...
  configDialog->addText(
      "All input clusters are merged in a preprocessing routine.");
  configDialog->addDouble("The cloud is firstly downscaled with voxel size  ",
                          " (m).", 0.01, 10, 4, _voxelSize);
  configDialog->addDouble(
      "Than an euclidean clustering routine is performed with threshold  ",
      " (m).", 0.01, 10, 4, _euclideanDistance);
  configDialog->addInt("A cluster has to contain at minimum ", " points.", 1,
                       9999, _minPts);
...
void SF_EuclideanClusteringSegmentationStep::
  ...
  if (resModelw != NULL) {
    resModelw->addGroupModel(DEF_IN_SCENE, _outGrpCluster,
                             new CT_StandardItemGroup(), tr("Segmentation"));
    resModelw->addItemModel(_outGrpCluster, _outCloudCluster, new CT_Scene(),
                            tr("Seeds Euclidean Cluster"));
  }
}

void SF_EuclideanClusteringSegmentationStep::createCTIndices(
...
void SF_EuclideanClusteringSegmentationStep::addResult(
    std::vector<std::vector<size_t>> &indexVec, CT_ResultGroup *outResult,
  ...
  while (!isStopped() && outResItOut.hasNext()) {
    ...
    for (size_t i = 0; i < CTindexVec.size(); i++) {
      if (CTindexVec[i]->size() > _minPts) {
        ...
        CTindexVec[i]->setSortType(
            CT_PointCloudIndexVector::SortedInAscendingOrder);
        CT_Scene *outScene =
...
void SF_EuclideanClusteringSegmentationStep::mergeCTClustersIntoPCL(
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloudPCL, std::vector<size_t> &indices,
    const QString &cloudStr, const QString &grpStr, CT_ResultGroup *outResult) {
  ...
  while (!isStopped() && outResIt.hasNext()) {
    ...
    const CT_AbstractItemDrawableWithPointCloud *ctCloud =
        (const CT_AbstractItemDrawableWithPointCloud *)
            group->firstItemByINModelName(this, cloudStr);
    if (first) {
...
void SF_EuclideanClusteringSegmentationStep::createParamList(
