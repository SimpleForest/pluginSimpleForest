...
QString
SF_EuclideanClusteringSegmentationStep::getStepDescription() const
{
  return tr("Segmentation Euclidean Clustering");
}

QString
SF_EuclideanClusteringSegmentationStep::getStepDetailledDescription() const
{
  return tr("Performs an Euclidean Clustering Operation to produce "
            "Segmentation Seeds.");
}

QString
SF_EuclideanClusteringSegmentationStep::getStepURL() const
{
  return tr("");
}

CT_VirtualAbstractStep*
SF_EuclideanClusteringSegmentationStep::createNewInstance(CT_StepInitializeData& dataInit)
{
  return new SF_EuclideanClusteringSegmentationStep(dataInit);
}

QStringList
SF_EuclideanClusteringSegmentationStep::getStepRISCitations() const
{
...
void
SF_EuclideanClusteringSegmentationStep::createInResultModelListProtected()
{
...
void
SF_EuclideanClusteringSegmentationStep::createPostConfigurationDialog()
{
  ...
  configDialog->addDouble(
    "Than an euclidean clustering routine is performed with threshold  ", " (m).", 0.01, 10, 4, _euclideanDistance);
...
void
SF_EuclideanClusteringSegmentationStep::createOutResultModelListProtected()
{
  CT_OutResultModelGroupToCopyPossibilities* resModelw = createNewOutResultModelToCopy(DEF_IN_RESULT);
  if (resModelw != NULL) {
    resModelw->addGroupModel(DEF_IN_SCENE, _outGrpCluster, new CT_StandardItemGroup(), tr("Segmentation"));
    resModelw->addItemModel(_outGrpCluster, _outCloudCluster, new CT_Scene(), tr("Seeds Euclidean Cluster"));
  }
}

void
SF_EuclideanClusteringSegmentationStep::createCTIndices(std::vector<CT_PointCloudIndexVector*>& CTindexVec,
                                                        std::vector<std::vector<size_t>>& indexVec)
{
  ...
  std::sort(CTindexVec.begin(), CTindexVec.end(), sfCompareCTCloudsBySize);
}

void
SF_EuclideanClusteringSegmentationStep::addResult(std::vector<std::vector<size_t>>& indexVec,
                                                  ...
                                                  const QString& uniqueName)
{
  ...
  CT_ResultGroupIterator outResItOut(outResult, this, uniqueName);
  while (!isStopped() && outResItOut.hasNext()) {
    ...
    for (size_t i = 0; i < CTindexVec.size(); i++) {
      if (CTindexVec[i]->size() > _minPts) {
        CT_StandardItemGroup* cloudGrp = new CT_StandardItemGroup(_outGrpCluster.completeName(), outResult);
        ...
        CT_Scene* outScene = new CT_Scene(
          _outCloudCluster.completeName(), outResult, PS_REPOSITORY->registerPointCloudIndex(CTindexVec[i]));
...
void
SF_EuclideanClusteringSegmentationStep::mergeCTClustersIntoPCL(pcl::PointCloud<pcl::PointXYZI>::Ptr cloudPCL,
                                                               std::vector<size_t>& indices,
                                                               const QString& cloudStr,
                                                               const QString& grpStr,
                                                               CT_ResultGroup* outResult)
{
  CT_ResultGroupIterator outResIt(outResult, this, grpStr);
  bool first = true;
...
void
SF_EuclideanClusteringSegmentationStep::compute()
{
  ...
  mergeCTClustersIntoPCL(cloudPCL, indices, cloudStr, grpStr, outResult);
...
void
SF_EuclideanClusteringSegmentationStep::createParamList(CT_ResultGroup* outResult)
{
