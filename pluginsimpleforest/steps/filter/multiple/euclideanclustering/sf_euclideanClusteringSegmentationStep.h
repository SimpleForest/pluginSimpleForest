...
class SF_EuclideanClusteringSegmentationStep : public SF_AbstractFilterMultipleStep
{
  Q_OBJECT
...
private:
  ...
  int _minPts = 1;
  void addResult(std::vector<std::vector<size_t>>& indexVec, CT_ResultGroup* outResult, const QString& uniqueName);
  void createCTIndices(std::vector<CT_PointCloudIndexVector*>& CTindexVec, std::vector<std::vector<size_t>>& indexVec);
