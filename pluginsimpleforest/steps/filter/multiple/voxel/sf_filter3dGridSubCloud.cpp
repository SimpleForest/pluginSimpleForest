...
SF_Filter3dGridSubCloud::SF_Filter3dGridSubCloud(CT_StepInitializeData &dataInit):
...
QString SF_Filter3dGridSubCloud::getStepDetailledDescription() const {
    return tr("Voxelises the cloud into a 3d Grid. Improves the runtime of various filters by enabling multithreading.");
}
...
void SF_Filter3dGridSubCloud::createPostConfigurationDialog() {
    ...
    configDialog->addDouble("The x,y,z voxelsize size of the output clouds ",
                             " ",
                             0.1,
                             10,
                             4,
...
CT_Grid3D_Sparse<int> * SF_Filter3dGridSubCloud::createGrid3dFromScene(const CT_Scene *ctCloud,
    ...
    CT_Grid3D_Sparse<int>* hitGrid = CT_Grid3D_Sparse<int>::createGrid3DFromXYZCoords(NULL,
                                                                                      NULL,
                                                                                      min(0),
                                                                                      min(1),
                                                                                      min(2),
...
void SF_Filter3dGridSubCloud::addPointToGridCluster(CT_Grid3D_Sparse<int>* hitGrid,
                                                    std::vector<CT_PointCloudIndexVector *> &clusters,
                                                    CT_PointIterator &it) {

    const CT_Point &point = it.next().currentPoint();
    int val = hitGrid->valueAtXYZ(point(0),
                                  point(1),
                                  point(2));
    createGridClusterIfNeeded(val,
                              clusters,
                              point,
                              hitGrid);
...
void SF_Filter3dGridSubCloud::addCloudToGridCluster(const CT_Scene* ctCloud,
                                                    std::vector<CT_PointCloudIndexVector *>& clusters) {
    CT_Grid3D_Sparse<int>* hitGrid = createGrid3dFromScene(ctCloud,
                                                           _voxelSize);
    const CT_AbstractPointCloudIndex *pointIndex = ctCloud->getPointCloudIndex();
    CT_PointIterator it(pointIndex);
    while(it.hasNext()) {
        addPointToGridCluster(hitGrid,
                              clusters,
                              it);
    }
}
...
void SF_Filter3dGridSubCloud::compute() {
    ...
    while(!isStopped() && outResIt.hasNext()) {
        ...
        const CT_Scene* ctCloud = (const CT_Scene*) group->firstItemByINModelName(this,
                                                                                  DEF_IN_CLOUD_SEED);
        addCloudToGridCluster(ctCloud,
                              clusters);
