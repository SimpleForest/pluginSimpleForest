...
QString SF_Filter3dGridSubCloud::getStepURL() const { return tr(""); }
...
void SF_Filter3dGridSubCloud::createOutResultModelListProtected() {
  ...
  if (resModelw != NULL) {
    resModelw->addGroupModel(DEF_IN_GRP_CLUSTER, _outGrpCluster,
                             new CT_StandardItemGroup(), tr("Grid"));
...
SF_Filter3dGridSubCloud::createGrid3dFromScene(const CT_Scene *ctCloud,
                                               double voxelSize) {
  ...
  CT_Grid3D_Sparse<int> *hitGrid =
      CT_Grid3D_Sparse<int>::createGrid3DFromXYZCoords(
          NULL, NULL, min(0), min(1), min(2), max(0), max(1), max(2), voxelSize,
          -2, -1);
  return hitGrid;
}

void SF_Filter3dGridSubCloud::createGridCluster(
...
void SF_Filter3dGridSubCloud::createGridClusterIfNeeded(
    int &val, std::vector<CT_PointCloudIndexVector *> &clusters,
    const CT_Point &point, CT_Grid3D_Sparse<int> *hitGrid) {
  if (val < 0) {
...
void SF_Filter3dGridSubCloud::addPointToGridCluster(
    CT_Grid3D_Sparse<int> *hitGrid,
...
void SF_Filter3dGridSubCloud::addCloudToGridCluster(
    const CT_Scene *ctCloud,
...
void SF_Filter3dGridSubCloud::compute() {
  ...
  while (!isStopped() && outResIt.hasNext()) {
    ...
    const CT_Scene *ctCloud = (const CT_Scene *)group->firstItemByINModelName(
