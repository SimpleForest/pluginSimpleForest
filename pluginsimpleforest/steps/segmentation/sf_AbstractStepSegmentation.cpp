...
SF_AbstractStepSegmentation::SF_AbstractStepSegmentation(CT_StepInitializeData &dataInit):
...
void SF_AbstractStepSegmentation::createPCLCloud(const QString& clusterGrpStr,
    ...
    while(!isStopped() && outResIt.hasNext()) {
        ...
        const CT_AbstractItemDrawableWithPointCloud* ctCloud =
                (const CT_AbstractItemDrawableWithPointCloud*) group->firstItemByINModelName(this, clusterStr);
        if(_first) {
...
int SF_AbstractStepSegmentation::getClusterNumber(pcl::PointCloud<pcl::PointXYZI>::Ptr cloudPCLDownscaled) {
    int size = 0;
...
void SF_AbstractStepSegmentation::downscale(pcl::PointCloud<pcl::PointXYZI>::Ptr cloudPCL,
                                    float voxelSize,
...
void SF_AbstractStepSegmentation::fillIndexVec(pcl::PointCloud<pcl::PointXYZI>::Ptr cloudPCL,
                                       std::vector<CT_PointCloudIndexVector *>& indexVec,
                                       std::vector<size_t>& indices,
                                       pcl::PointCloud<pcl::PointXYZI>::Ptr cloudPCLDownscaled,
                                       float maxRange) {
