...
QString SF_StepSegmentationDijkstra::getStepURL() const { return tr(""); }

CT_VirtualAbstractStep *SF_StepSegmentationDijkstra::createNewInstance(
    CT_StepInitializeData &dataInit) {
...
void SF_StepSegmentationDijkstra::createInResultModelListProtected() {
  ...
  resModel->addItemModel(DEF_IN_GRP_CLUSTER, DEF_IN_CLOUD_SEED,
                         CT_Scene::staticGetType(), tr("Vegetation Cloud"));
  resModel->addGroupModel(
      "", DEF_IN_SCENE, CT_AbstractItemGroup::staticGetType(), tr("Seed Group"),
      "", CT_InAbstractGroupModel::CG_ChooseOneIfMultiple);
  resModel->addItemModel(DEF_IN_SCENE, DEF_IN_SCENE_CLOUD,
                         CT_Scene::staticGetType(), tr("Seed Cloud"));
}

void SF_StepSegmentationDijkstra::createPostConfigurationDialog() {
  CT_StepConfigurableDialog *configDialog =
      newStandardPostConfigurationDialog();
  configDialog->addDouble(
      "The cloud is scaled along the z axis with a factor of  ", ".", 0.1, 1, 1,
      _zFactor);
  configDialog->addDouble("The cloud is downscaled with voxel size  ", " (m).",
                          0.01, 10, 4, _voxelSize);
  configDialog->addDouble("Then a Dijkstra based Segmentation is performed "
                          "with neighbors connection with range  ",
                          " (m).", 0.01, 10, 4, _euclideanDistance);
...
void SF_StepSegmentationDijkstra::createOutResultModelListProtected() {
  ...
  if (resModelw != NULL) {
    resModelw->addItemModel(DEF_IN_SCENE, _outCloudCluster, new CT_Scene(),
...
void SF_StepSegmentationDijkstra::compute() {
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr cloudPCLDownscaled(
      new pcl::PointCloud<pcl::PointXYZI>);
  pcl::PointCloud<pcl::PointXYZI>::Ptr clustersPCL(
      new pcl::PointCloud<pcl::PointXYZI>);
  pcl::PointCloud<pcl::PointXYZI>::Ptr clustersPCLDownscaled(
      new pcl::PointCloud<pcl::PointXYZI>);
  std::vector<size_t> indices;
  std::vector<size_t> indicesCluster;
  std::vector<CT_PointCloudIndexVector *> indexVec;

  createPCLCloud(DEF_IN_GRP_CLUSTER, DEF_IN_CLOUD_SEED, outResult, cloudPCL,
                 indices, _zFactor);
  createPCLCloud(DEF_IN_SCENE, DEF_IN_SCENE_CLOUD, outResult, clustersPCL,
                 indicesCluster, _zFactor);
  downscale(cloudPCL, _voxelSize, cloudPCLDownscaled);
  downscale(clustersPCL, _voxelSize, clustersPCLDownscaled);
  SF_Dijkstra djik(cloudPCLDownscaled, clustersPCLDownscaled,
                   _euclideanDistance);
  ...
  CT_ColorCloudStdVector *_colors =
      new CT_ColorCloudStdVector(cloudPCL->points.size());
  ...
  CT_ResultGroupIterator resultGrpIterator2(outResult, this,
                                            DEF_IN_GRP_CLUSTER);
  while (!isStopped() && resultGrpIterator2.hasNext()) {
    CT_StandardItemGroup *group =
        (CT_StandardItemGroup *)resultGrpIterator2.next();
    ...
    CT_PointsAttributesColor *colorAttribute = new CT_PointsAttributesColor(
        ...
        ctCloud->getPointCloudIndexRegistered(), _colors);
  ...
  while (!isStopped() && resultGrpIterator.hasNext()) {
    CT_StandardItemGroup *group =
        (CT_StandardItemGroup *)resultGrpIterator.next();
    indexVec[i]->setSortType(CT_PointCloudIndexVector::SortedInAscendingOrder);
    CT_Scene *outScene =
        new CT_Scene(_outCloudCluster.completeName(), outResult,
