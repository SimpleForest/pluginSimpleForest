...
SF_StepSegmentationDijkstra::SF_StepSegmentationDijkstra(CT_StepInitializeData& dataInit) : SF_AbstractStepSegmentation(dataInit) {}

SF_StepSegmentationDijkstra::~SF_StepSegmentationDijkstra() {}

QString
SF_StepSegmentationDijkstra::getStepDescription() const
{
  return tr("Dikstra Based Tree Segmentation from seeds");
}

QString
SF_StepSegmentationDijkstra::getStepDetailledDescription() const
{
  return tr("The step takes vegetation points and seed clusters as input. All "
            "clusters are tagged with an own id and zero distance, remaining "
            "non cluster points are"
            "initialized with infinity distance. Then a competitive dijkstra "
            "is applied. Later all points are tagged with the id from the seed "
            "cluster they are connected to."
            "Before the routine the cloud can be scaled in z-axis to enable "
            "easier vertical growth of the dijkstra routine."
            "If points remain not reached by the dijkstra they are alligned to "
            "the closest pre cluster tree by nearest neighbor check.");
}

QString
SF_StepSegmentationDijkstra::getStepURL() const
{
  return tr("");
}

CT_VirtualAbstractStep*
SF_StepSegmentationDijkstra::createNewInstance(CT_StepInitializeData& dataInit)
{
  return new SF_StepSegmentationDijkstra(dataInit);
}

QStringList
SF_StepSegmentationDijkstra::getStepRISCitations() const
{
  ...
  _RIS_citation_list.append(QString("TY  - JOUR\n"
                                    ...
                                    "download?doi=10.1.1.438.1610&rep=rep1&type=pdf\n"
                                    "ER  - \n"));
...
void
SF_StepSegmentationDijkstra::createInResultModelListProtected()
{
  ...
  resModel->addItemModel(DEF_IN_GRP_CLUSTER, DEF_IN_CLOUD_SEED, CT_Scene::staticGetType(), tr("Vegetation Cloud"));
  resModel->addGroupModel(
    "", DEF_IN_SCENE, CT_AbstractItemGroup::staticGetType(), tr("Seed Group"), "", CT_InAbstractGroupModel::CG_ChooseOneIfMultiple);
  resModel->addItemModel(DEF_IN_SCENE, DEF_IN_SCENE_CLOUD, CT_Scene::staticGetType(), tr("Seed Cloud"));
}

void
SF_StepSegmentationDijkstra::createPostConfigurationDialog()
{
  CT_StepConfigurableDialog* configDialog = newStandardPostConfigurationDialog();
  configDialog->addDouble("The cloud is scaled along the z axis with a factor of  ", ".", 0.1, 1, 1, _zFactor);
  configDialog->addDouble("The cloud is downscaled with voxel size  ", " (m).", 0.01, 10, 4, _voxelSize);
  configDialog->addDouble("Then a Dijkstra based Segmentation is performed "
                          "with neighbors connection with range  ",
                          " (m).",
                          0.01,
                          10,
                          4,
                          _euclideanDistance);
  configDialog->addText("Voxel size should be at minimum 2 or 3 times smaller "
                        "than Dijkstra range.");
  createPostConfigurationDialogCitation(configDialog);
}

void
SF_StepSegmentationDijkstra::createOutResultModelListProtected()
{
  CT_OutResultModelGroupToCopyPossibilities* resModelw = createNewOutResultModelToCopy(DEF_IN_RESULT);
  if (resModelw != NULL) {
    resModelw->addItemModel(DEF_IN_SCENE, _outCloudCluster, new CT_Scene(), tr("Dijsktra Segmented"));
    resModelw->addItemModel(DEF_IN_GRP_CLUSTER, m_outCloudItem, new CT_PointsAttributesColor(), tr("Dijkstra distance"));
  }
}

void
SF_StepSegmentationDijkstra::compute()
{
  ...
  createPCLCloud(DEF_IN_GRP_CLUSTER, DEF_IN_CLOUD_SEED, outResult, cloudPCL, indices, _zFactor);
  createPCLCloud(DEF_IN_SCENE, DEF_IN_SCENE_CLOUD, outResult, clustersPCL, indicesCluster, _zFactor);
  downscale(cloudPCL, _voxelSize, cloudPCLDownscaled);
  downscale(clustersPCL, _voxelSize, clustersPCLDownscaled);
  SF_Dijkstra djik(cloudPCLDownscaled, clustersPCLDownscaled, _euclideanDistance);
  ...
  CT_ResultGroupIterator resultGrpIterator2(outResult, this, DEF_IN_GRP_CLUSTER);
  while (!isStopped() && resultGrpIterator2.hasNext()) {
    ...
    CT_PointsAttributesColor* colorAttribute = new CT_PointsAttributesColor(
      m_outCloudItem.completeName(), outResult, ctCloud->getPointCloudIndexRegistered(), _colors);
  ...
  while (!isStopped() && resultGrpIterator.hasNext()) {
    ...
    CT_Scene* outScene = new CT_Scene(_outCloudCluster.completeName(), outResult, PS_REPOSITORY->registerPointCloudIndex(indexVec[i]));
...
void
SF_StepSegmentationDijkstra::createParamList(CT_ResultGroup* outResult)
{
  outResult->getName();
}
