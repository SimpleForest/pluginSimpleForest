...
template <typename PointType>
void SF_PCA<PointType>::setParameters(float range, bool centerZero,
                                      bool useRange) {
  _range = range;
...
template <typename PointType>
void SF_PCA<PointType>::setParameters(int k, bool centerZero) {
  _k = k;
  _centerZero = centerZero;
  _k = 0.03;
  _useRange = false;
...
template <typename PointType>
void SF_PCA<PointType>::extractNeighbors(
    typename pcl::KdTree<PointType>::Ptr kdTree, PointType p,
    typename pcl::PointCloud<PointType>::Ptr neighborhood) {
  if (_useRange) {
    SF_AbstractCloud<PointType>::extractNeighborsByRange(kdTree, p,
                                                         neighborhood, _range);
  } else {
    SF_AbstractCloud<PointType>::extractNeighborsByKnn(kdTree, p, neighborhood,
                                                       _k);
  }
}

template <typename PointType>
SF_PCAValues SF_PCA<PointType>::computeFeaturesFromNeighbors(
    typename pcl::PointCloud<PointType>::Ptr neighborhood,
...
template <typename PointType>
void SF_PCA<PointType>::computeFeaturesForPoint(
    const PointType &p, typename pcl::KdTree<PointType>::Ptr kdTree,
    int index) {
  ...
  SF_PCAValues ppca = computeFeaturesFromNeighbors(
      neighborhood, (_centerZero ? origin : xyz_centroid));
  pcaValues[index] = ppca;
}

template <typename PointType> void SF_PCA<PointType>::computeFeatures() {
  if (pcaValues.size() != SF_PCA<PointType>::_cloudIn->points.size()) {
    pcaValues.resize(SF_PCA<PointType>::_cloudIn->points.size());
  }

  typename pcl::KdTreeFLANN<PointType>::Ptr kd_tree(
      new typename pcl::KdTreeFLANN<PointType>());
