...
SF_GrowthDirection<PointType, FeatureType>::SF_GrowthDirection(
...
template <typename PointType, typename FeatureType>
void SF_GrowthDirection<PointType, FeatureType>::setParameters(
    float rangeNormal, float rangeGd) {
...
SF_GrowthDirection<PointType, FeatureType>::computeNormalPca() {
...
template <typename PointType, typename FeatureType>
void SF_GrowthDirection<PointType, FeatureType>::addNormals(
    std::vector<SF_PCAValues> &values) {
  for (size_t i = 0; i < values.size(); i++) {
    SF_PCAValues val = values[i];
    SF_AbstractCloud<PointType>::_cloudIn->points[i].normal_x =
        val.getVector3()[0];
    SF_AbstractCloud<PointType>::_cloudIn->points[i].normal_y =
        val.getVector3()[1];
    SF_AbstractCloud<PointType>::_cloudIn->points[i].normal_z =
        val.getVector3()[2];
  }
}

template <typename PointType, typename FeatureType>
void SF_GrowthDirection<PointType, FeatureType>::addGrowthDirection(
    std::vector<SF_PCAValues> &values) {
  for (size_t i = 0; i < values.size(); i++) {
    FeatureType pointGd;
    ...
    if (val.getLambda1() > MAX_LAMBDA3) {
      ...
      SF_AbstractCloud<PointType>::extractNeighborsByRange(
          _kdTree, p, neighborhood, _rangeGd);
      ...
      SF_PCAValues valGd =
          SF_PCA<PointType>::computeFeaturesFromNeighbors(neighborhood, origin);
      pointGd.normal_x = valGd.getVector3()[0];
      pointGd.normal_y = valGd.getVector3()[1];
      pointGd.normal_z = valGd.getVector3()[2];
    }
    SF_AbstractFeature<PointType, FeatureType>::_featuresOut->points[i] =
        pointGd;
  }
}
...
void SF_GrowthDirection<PointType, FeatureType>::computeFeatures() {
  if (SF_GrowthDirection::_featuresOut->points.size() !=
      SF_GrowthDirection::_cloudIn->points.size()) {
    SF_GrowthDirection::_featuresOut->resize(
        SF_GrowthDirection::_cloudIn->points.size());
  }
