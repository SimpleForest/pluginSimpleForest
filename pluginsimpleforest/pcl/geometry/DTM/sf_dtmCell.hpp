...
void SF_DTMCell<PointType>:: updateMinMax(const float updateHeight, float &minHeight, float& maxHeight) {
...
template<typename PointType>
float SF_DTMCell<PointType>::getHeight(const Eigen::Vector2d &coords, const pcl::ModelCoefficients &coeff) {
    float height = (coords[0]*coeff.values[0] + coords[1]*coeff.values[1] + coeff.values[3])/-coeff.values[2];
    return height;
}

template<typename PointType>
Eigen::Vector2f SF_DTMCell<PointType>::getMinMaxHeight(const pcl::ModelCoefficients &coeff) {
