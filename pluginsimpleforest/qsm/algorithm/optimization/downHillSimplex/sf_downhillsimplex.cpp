...
SF_ParamSpherefollowingBasic<SF_PointNormal>
...
void
SF_DownHillSimplex::setParams(const SF_ParamSpherefollowingBasic<SF_PointNormal>& params)
{
  m_params = params;
}
...
void
SF_DownHillSimplex::serializeParams(std::uintptr_t* par)
{
...
void
SF_DownHillSimplex::serializeVec(gsl_vector* x, double fac)
{
  size_t index = 0;
...
SF_DownHillSimplex::compute()
{
  ...
  gsl_vector *ss, *x;
  x = gsl_vector_alloc(m_params.m_numClstrs * 3);
  ss = gsl_vector_alloc(m_params.m_numClstrs * 3);
  ...
  serializeVec(x, 1.0);
  serializeVec(ss, 0.2);
  minex_func.n = m_params.m_numClstrs * 3;
  minex_func.f = downhillSimplex;
  minex_func.params = par;
  s = gsl_multimin_fminimizer_alloc(T, m_params.m_numClstrs * 3);
...
downhillSimplex(const gsl_vector* v, void* params)
{
  ...
  paramsBasic._cloudIn.reset(reinterpret_cast<pcl::PointCloud<pcl::PointXYZINormal>*>(p[0]),
