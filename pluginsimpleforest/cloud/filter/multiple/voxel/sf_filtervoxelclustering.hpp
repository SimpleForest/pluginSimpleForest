...
template<typename PointType>
void
SF_VoxelClustering<PointType>::compute()
{
  initialize();
  std::unique_ptr<CT_Grid3D_Sparse<int>> clusterIndices(CT_Grid3D_Sparse<int>::createGrid3DFromXYZCoords(
    NULL, NULL, m_min.x, m_min.y, m_min.z, m_max.x, m_max.y, m_max.z, m_param.m_voxelSize, -2, -1));
  ...
  for (size_t i = 0; i < m_param.m_cloud.first->points.size(); i++) {
    ...
    if (clusterIndex <= -1) {
      ...
      SF_AbstractMultipleFilter<PointType>::m_clusterOut.push_back(
        std::pair<typename pcl::PointCloud<PointType>::Ptr, std::vector<size_t>>(cluster, CT_indices));
...
template<typename PointType>
void
SF_VoxelClustering<PointType>::initialize()
{
...
template<typename PointType>
void
SF_VoxelClustering<PointType>::setParam(const SF_ParameterSetVoxelization<PointType>& param)
{
  m_param = param;
}
