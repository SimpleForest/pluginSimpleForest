...
template <typename PointType> void SF_VoxelGridDownscale<PointType>::compute() {
  ...
  std::vector<
      ...
      clusters = vc.clusterOut();
  auto centroids = computeCentroids(clusters);
  downScale(clusters, centroids);
}

template <typename PointType>
void SF_VoxelGridDownscale<PointType>::setParams(
    SF_ParameterSetVoxelgridDownscaling<PointType> &params) {
  m_param = params;
}
...
SF_VoxelGridDownscale<PointType>::computeCentroids(
    const std::vector<std::pair<typename pcl::PointCloud<PointType>::Ptr,
...
template <typename PointType>
void SF_VoxelGridDownscale<PointType>::downScale(
    const std::vector<std::pair<typename pcl::PointCloud<PointType>::Ptr,
                                std::vector<size_t>>> &clusters,
    typename pcl::PointCloud<PointType>::Ptr centroids) {
  typename pcl::PointCloud<PointType>::Ptr cloudOne(
      new pcl::PointCloud<PointType>);
  typename pcl::PointCloud<PointType>::Ptr cloudTwo(
      new pcl::PointCloud<PointType>);
  ...
  std::pair<typename pcl::PointCloud<PointType>::Ptr, std::vector<size_t>>
      pairOne(std::move(cloudOne), std::move(CTIndicesOne));
  std::pair<typename pcl::PointCloud<PointType>::Ptr, std::vector<size_t>>
      pairTwo(std::move(cloudTwo), std::move(CTIndicesTwo));
