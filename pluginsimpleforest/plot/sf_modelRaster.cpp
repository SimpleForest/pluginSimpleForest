...
void SF_ModelRaster::setCloud3D(const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud3D) {
    _cloud3D = cloud3D;
    create2DFrom3D();
    createKDTree();
}
...
void SF_ModelRaster::getNearestNeighborsHeightsAndDistances(const pcl::PointXYZ &point, const int nn,
...
void SF_ModelRaster::interpolateIDW(const int knn, std::shared_ptr<SF_ModelRaster> dst) {
...
void SF_ModelRaster::interpolateMedian(const int knn, std::shared_ptr<SF_ModelRaster> dst) {
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloudDst = dst->getCloud3D();
    for(size_t i = 0; i < cloudDst->points.size(); i++) {
        pcl::PointXYZ point = cloudDst->points[i];
        point.z = 0;
