...
#include "sf_raster_model.h"
#include "math/interpolation/sf_interpolation.h"

pcl::PointCloud<pcl::PointXYZ>::Ptr SF_Raster_Model::getCloud3D() const {
    return _cloud3D;
}

void SF_Raster_Model::setCloud3D(const pcl::PointCloud<pcl::PointXYZ>::Ptr cloud3D) {
    _cloud3D = cloud3D;
    create2DFrom3D();
    createKDTree();
}

void SF_Raster_Model::create2DFrom3D() {
...
void SF_Raster_Model::createKDTree() {
    _kdtree2D.reset(new pcl::KdTreeFLANN<pcl::PointXYZ>);
    _kdtree2D->setInputCloud(_cloud2D);
}

void SF_Raster_Model::getNearestNeighborsHeightsAndDistances(const pcl::PointXYZ &point, const int nn,
                                                             std::vector<float> &heightsOut, std::vector<float> &distancesOut) {
    heightsOut.clear();
    distancesOut.clear();
...
void SF_Raster_Model::interpolateIDW(const int knn, std::shared_ptr<SF_Raster_Model> dst) {
...
void SF_Raster_Model::interpolateMedian(const int knn, std::shared_ptr<SF_Raster_Model> dst) {
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloudDst = dst->getCloud3D();
    for(size_t i = 0; i < cloudDst->points.size(); i++) {
        pcl::PointXYZ point = cloudDst->points[i];
        point.z = 0;
...
float SF_Raster_Model::heightAt(float x, float y) {
...
SF_Raster_Model::SF_Raster_Model(pcl::PointCloud<pcl::PointXYZ>::Ptr cloud3D) {
    _cloud3D = cloud3D;
    create2DFrom3D();
    createKDTree();
}
