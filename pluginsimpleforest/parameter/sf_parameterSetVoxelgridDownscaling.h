...
struct SF_ParameterSetVoxelgridDownscaling : public SF_AbstractParameterSet<T> {
  ...
  float m_voxelSize;
  /**
   * @brief m_cloudOut Contains one cloud containing the remaining points and
   * one cloud containing the filtered out points.
   */
...
private:
  QStringList paramsToString() override { return QStringList(); }
};
